...
public class GnuPG {
	...
	// Konstants:
	private final String kGnuPGCommand = "gpg --batch --armor --output -";
	
	// Class vars:
	private File	tmpFile;
	private int		gpg_exitCode = -1;
	public String	gpg_result;
	...
	public boolean sign (String inStr, String passPhrase)
	{
		...
		success = createTempFile (inStr, ".tmp");
	...
	public boolean decrypt (String inStr, String passPhrase)
	{
		...
		success = createTempFile (inStr, ".txt.gpg");
		if (success)
		{
			String fileName = this.tmpFile.getAbsolutePath ().substring(36);
			System.out.println(fileName);
			success = runGnuPG ("--passphrase-fd 0 --decrypt " + fileName, passPhrase);
			this.tmpFile.delete ();
			if (success && this.gpg_exitCode != 0)
				success = false;
		}
...
//		String		fullCommand = commandArgs;
		...
		psr_stdout.start();
		psr_stderr.start();
		System.out.println("1"+psr_stdout.getString());
		System.out.println("2"+psr_stderr.getString());
		if (inputStr != null)
		{
		...
		System.out.println("3"+psr_stdout.getString());
		System.out.println("4"+psr_stderr.getString());
		try
		{
			p.waitFor();
		...
		System.out.println("5"+psr_stdout.getString());
		System.out.println("6"+psr_stderr.getString());
	 ...
	 * @return			true if success
	 */
	private boolean createTempFile (String inStr, String suffix)
	{
		this.tmpFile = null;
		FileWriter	fw;
		...
		try
		{
			this.tmpFile = File.createTempFile ("YGnuPG", suffix);
		}
		catch (Exception e)
		{
		...
		{
			System.out.println("Exception -> tmpFile deleted");
			// delete our file:
			tmpFile.delete ();
